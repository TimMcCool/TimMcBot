...
class leveling(commands.Cog):
  ...
  async def on_message(self, message):
    ...
    change_xp = (round(time.time())) - data["last"] > 60 and not message.content.startswith(tuple(prefixes))
    if change_xp:
      ...
      if old_level != new_level:
        await message.channel.send(f"GG {message.author.mention}, you just advanced to **Level {new_level}**! Keep it up! :partying_face:")
      data["last"] = round(time.time())
      if data["daily"]["day"] == (datetime.today() - datetime.utcfromtimestamp(0)).days: data["daily"] = {
      ...
      if data["weekly"]["week"] == math.floor(((datetime.today() - datetime.utcfromtimestamp(0)).days - 4)/7): data["weekly"] = {
        "xp" : data["weekly"]["xp"]+new_xp,
        "week" : math.floor(((datetime.today() - datetime.utcfromtimestamp(0)).days - 4)/7)
        }
      else: data["weekly"] = {
        "xp" : new_xp,
        "week" : math.floor(((datetime.today() - datetime.utcfromtimestamp(0)).days - 4)/7)
        ...
        if str(message.guild.id) in levelroles:
          ...
          if str(new_rank) in guild_lr:
            ...
            for index in guild_lr:
              ...
              try:
                member = message.guild.get_member(int(t[int(index)-1][1]))
                try:
  ...
  @commands.command(brief="Shows the rank card of a user", aliases=["r", "ranking","rang"])
  ...
  async def _levels(self, ctx, type):
    mode = int(type)
    if mode == 3:
      await ctx.send(f"Here is **{ctx.guild.name}**'s web leaderboard:\nhttps://timmcbot.tim135790.repl.co/leaderboard/?guild={ctx.guild.id}")
    else:
      await levellist(self, ctx, mode=mode)

  @commands.group(brief="Shows the server's leaderboard", aliases=["leaderboard","lb","leaders","ranklist","rangliste"])
  ...
  @commands.group(brief="Shows the rank role settings", aliases=["rankrole","levelroles","lbroles"])
  async def rankroles(self, ctx):
    if ctx.invoked_subcommand is None:
      with open("json_files/levelroles.json", "r") as d:
        levelroles = json.load(d)
      embed = discord.Embed(title="Rank roles", description=f"will be automatically given to members with a specific TimMcBot rank\n{emojis['spacer']}", color=get_client_color(ctx))
      ...
      embed.add_field(name="How to set a rank role:", value="```+rankroles set <rank> <role>```", inline=False)
      embed.add_field(name="How to remove the rank role of a rank:", value="```+rankroles remove <rank>```", inline=False)
      embed.set_footer(text="You need the 'Manage Server' permission to set rank roles.")
      embed.set_author(name=f"🎭 {ctx.message.guild.name}", icon_url=ctx.message.guild.icon_url)
  ...
  async def set(self, ctx, rank : int, *, role : discord.Role):
    ...
    if role.managed is True:
      await ctx.send(embed=discord.Embed(title="Error", description="This role can't be used because it is managed by a bot or by an intergration.", color=discord.Color.red()))
      ...
      await ctx.send(f"**Success!** The role {origin_role.mention} will be automatically assigned to the member with rank **{origin_rank}.**")
  ...
  async def remove(self, ctx, rank : int):
    ...
    if str(ctx.guild.id) in levelroles:
      if str(rank) in levelroles[str(ctx.guild.id)]:
        ...
        await ctx.send(f"**Success!** The rank role for rank **{rank}** has been removed.")
        await ctx.message.add_reaction(emojis['checkmark'])
...
async def levellist(self, ctx, *, mode):
  ...
  if mode == 0:
    ...
    def check(elem):
      try: return elem[0]['weekly']['xp']
      except IndexError: return 0

    t = sorted(tbefore, key=check, reverse=True)
    
    def check(elem):
      try: return elem[0]['weekly']['week'] == math.floor(((datetime.today()  - datetime.utcfromtimestamp(0)).days - 4)/7)
    ...
    def check(elem):
      try: return elem[0]['daily']['xp']
      except IndexError: return 0

    t = sorted(tbefore, key=check, reverse=True)
    
    def check(elem):
      try: return elem[0]['daily']['day'] == (datetime.today() - datetime.utcfromtimestamp(0)).days
      except Exception: return False
  ...
  if len(t) < 6:
    ...
    seite = 1
    tcropped = t[0:5]
    board = await levelcard(self, ctx, tcropped, f"📖 Page {seite} of {seiten}", 0, symbol, mode)
    if random.randint(0,5)==0 and mode == 0:
      message = await ctx.send(f"**Tip:** Enter `{prefix[1]}levels weekly` to see the weekly leaderboard. :bulb:", embed=board)
    elif random.randint(0,5)==1 and mode == 0:
      message = await ctx.send(f"**Tip:** Enter `{prefix[1]}levels daily` to see the daily leaderboard. :bulb:", embed=board)
    else: message = await ctx.send(embed=board)
    await message.add_reaction("⏮"), await message.add_reaction("◀"), await message.add_reaction("▶"), await message.add_reaction("⏭")
    while True:

      def check(reaction, user):
        return reaction.message == message and str(reaction.emoji) in ["⏮","◀","▶","⏭"] and user == ctx.author
      ...
      try:
        ...
        board = await levelcard(self, ctx, tcropped, f"📖 Page {seite} of {seiten}", (seite-1)*5, symbol, mode)
        await message.edit(embed=board)
        await reaction.remove(user)

async def levelcard(self, ctx, tcropped, footer, low_rank, symbol, mode):
  if mode == 0: content="The most active server members:\n\n"
  elif mode == 1: content="This week's most active server members:\n\n"
  elif mode == 2: content="Today's most active server members:\n\n"
  i=0
  for item in tcropped:
    ...
    elif mode == 1: userinfo = f" {symbol} **{item[0]['name']}**\n{emojis['spacer']}{emojis['coin']} `{item[0]['weekly']['xp']}`\n\n"
    elif mode == 2: userinfo = f" {symbol} **{item[0]['name']}**\n{emojis['spacer']}{emojis['coin']} `{item[0]['daily']['xp']}`\n\n"
    if i+low_rank==1: content=f"{content}**:first_place:**{userinfo}"
    elif i+low_rank==2: content=f"{content}**:second_place:**{userinfo}"
    elif i+low_rank==3: content=f"{content}**:third_place:**{userinfo}"
    else: content=f"{content}**#{i+low_rank}**{userinfo}"
  if mode == 0:
    content=content+f":small_orange_diamond: [Click here](https://timmcbot.tim135790.repl.co/leaderboard/?guild={ctx.guild.id}) to visit the **web leaderbaord.**"
  elif mode == 1:
    today = now = datetime.today()
    today = datetime(today.year, today.month, today.day)
    next_reset = datetime.today() + timedelta(days=(timedelta(days=7-now.weekday()) + today - now).days+1)
    content=content+f":small_orange_diamond: **Next reset:** Monday ({next_reset.strftime('%m/%d/%Y')}) at 12am UTC"
  elif mode == 2:
    content=content+":small_orange_diamond: **Next reset:** Tomorrow at 12am UTC"
  board = discord.Embed(title=ctx.guild.name, description=content, color= get_client_color(ctx))
...
async def rankcard(ctx, user, myself):
  ...
  if str(ctx.guild.id) in servers:
    ...
    if str(user.id) in users: 
      ...
      content=f"{content}**Messages: {umessages['messages']}**{emojis['spacer']}"
      content=f"{content}**{umessages['xp']}** / {levelup+1} {emojis['coin']} {emojis['spacer']}"
      if rank==1: content=f"{content}:first_place: **Rank: #{rank}**"
      elif rank==2: content=f"{content}:second_place: **Rank: #{rank}**"
      elif rank==3: content=f"{content}:third_place: **Rank: #{rank}**"
      else: content=f"{content}:trophy: **Rank: #{rank}**"
      card = discord.Embed(title="Rank Card", description=content, color=discord.Colour.random())
      card.set_author(name="👤 "+str(user), icon_url=str(user.avatar_url))
      card.set_footer(text=f"Server: {ctx.guild.name}", icon_url=str(ctx.guild.icon_url))
      random_number=random.randint(0,4)
      if random_number == 0:
        await ctx.send("**Tip:** To prevent spamming, you can only gain XP once every minute. :bulb:", embed=card)
      elif random_number == 1:
        await ctx.send("**Tip:** You don't get XP for TimMcBot commands! :bulb:", embed=card)
      else: await ctx.send(embed=card)
    else:
      if myself is True:
        await ctx.send("You aren't ranked yet! Send some messages first, then try again.")
      else:
