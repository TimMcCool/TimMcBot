...
class fun(commands.Cog):
    ...
    @commands.command(aliases=["die", "würfel", "würfeln"],
                      brief="Rolls the dice",
                      description="Rolls the dice.")
    async def dice(self, ctx):
        zahlen = ["one", "two", "three", "four", "five", "six"]
        if ctx.author.nick == None: user = ctx.author.name
        else: user = ctx.author.nick
        if randint(0, 6) == 6:
            await ctx.send(
                embed=discord.Embed(
                    description=
                    f"☹ **{user} wanted to roll their dice, but then realized that they forgot it at home!** rip",
                    color=discord.Colour.random()))
        else:
            await ctx.send(
                embed=discord.Embed(
                    description=
                    f"🎲 **{user} rolled a {random.choice(zahlen)}!**",
                    color=discord.Colour.random()))

    @commands.command(aliases=["f-in-the-chat","f"])
    @commands.cooldown(3, 30, commands.BucketType.user)
    async def pressf(self, ctx, *, occurrence):
      message = await ctx.send(f"**{occurrence}** - React with {emojis['f_in_the_chat']} to pay respects!")
      await message.add_reaction(emojis['f_in_the_chat'])
      reactions = 0
      reactors = []

      def check(reaction, user):
        return reaction.message == message and not user.bot and str(reaction.emoji) == emojis['f_in_the_chat']

      while True:
        try: reaction, user = await self.client.wait_for('reaction_add', timeout=300.0, check=check)
        except asyncio.TimeoutError:
          if reactions == 0: await ctx.send("Time's up! :alarm_clock: **Nobody** paid their respects. :slight_frown:")
          else: await ctx.send(f"Time's up! :alarm_clock: The following people paid their respects: **{', '.join(reactors)}**")
          return
        else:
          reactions += 1
          if user.name in reactors:
            await ctx.send(f"{user.mention} paid their respects **again!** {emojis['f_in_the_chat']} They seem to be really compassionate ...")
          else:
            reactors.append(user.name)
            await ctx.send(f"{user.mention} paid their respects! {emojis['f_in_the_chat']}")
    ...
    async def hardcorespoiler(self, ctx, *, text=None):
      ...
      webhook = await ctx.channel.create_webhook(name=ctx.channel.id, reason="A slash command that requires a webhook was run")
      spoilered = await hardcore_spoiler(ctx, message)
      if ctx.author.nick is None:
        await webhook.send(spoilered, username=ctx.author.name, avatar_url=ctx.author.avatar_url)
      else:
        await webhook.send(spoilered, username=ctx.author.nick, avatar_url=ctx.author.avatar_url)
    ...
    async def _rickroll(self, ctx, target=None):
      if target is None:
        await ctx.send("https://media.tenor.com/images/59de4445b8319b9936377ec90dc5b9dc/tenor.gif")
      else:
        await ctx.send(
          f"{target.mention} {ctx.author.name} has gifted you **{random.choice(rickroll_wins)}!** :tada:",
          embed=discord.Embed(description="**[Click here to claim it!](http://alturl.com/7y5we)** :boom:", color=discord.Colour.blue())
          ) 
    ...
    async def clap(self, ctx, *, text=None):
        ...
        for i in text:
            o = o + 1
            if o == len(text): output = f"{output}{i}"
            else: output = f"{output}{i}  :clap: "
    ...
    @commands.command(brief="🇪 🇲 🇴 🇯 🇮 🇫 🇮 🇪 🇸 your message")
    async def emojify(self, ctx, *, text=None):
      await ctx.message.delete()
      webhook = await ctx.channel.create_webhook(name=ctx.channel.id, reason="A slash command that requires a webhook was run")
      emojified = await emojify(ctx, text)
      if ctx.author.nick is None:
        await webhook.send(emojified, username=ctx.author.name, avatar_url=ctx.author.avatar_url)
      else:
        await webhook.send(emojified, username=ctx.author.nick, avatar_url=ctx.author.avatar_url)
      await webhook.delete()
