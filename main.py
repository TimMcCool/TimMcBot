...
def get_prefix(client, message):
  ...
  try:
    if str(message.guild.id) in serverdata:
      ...
      return prefixes
    else: return ["<@!800377812699447306> ","+"]
...
#init
client = commands.Bot(command_prefix=get_prefix, intents=discord.Intents.all(), allowed_mentions = discord.AllowedMentions(roles=False, users=True, everyone=False))
...
async def _help(ctx, category=None, command=None):
  ...
  if not embed is None: await ctx.send(embed=embed)
...
async def help_home(ctx, prefix):
  ...
  for item in cogs:
    if item in categories: embed.description = embed.description+"\n\n**"+categories[item]+"** 🔹 "+f"`{prefix}help {item}`"
  embed.description += f"\n\n:small_orange_diamond: There are **slash commands** too! Type / to see them."
  return embed

async def help_cog(ctx, cog_name, prefix):
  if cog_name in categories: name = categories[cog_name]
  elif cog_name is None: name = "📁 Other"
  else: name = cog_name
  ...
  for command in client.commands:
    if command.cog is None: this_cog_name = None
    else: this_cog_name = command.cog.qualified_name
    params = ""
    if this_cog_name == cog_name and not command.hidden:
      ...
      commandinfo = ""
      if not command.brief is None: commandinfo += command.brief+"\n"
      try:
        subcommands = None
      ...
      if commandinfo == "":
        embed.add_field(name=prefix+command.qualified_name+params, value="** **", inline=False)
      else:
        embed.add_field(name=prefix+command.qualified_name+params, value="```"+commandinfo+"```", inline=False)
  embed.description += f"Enter `{prefix}help <command>` to get info on a certain command"
  return embed
  ...
  if command is None or command.hidden:
      ...
      for alt in command.aliases:
        if command.full_parent_name == "":
          if alts == None: alts = f"{prefix}{alt}{params}"
          else: alts = f"{alts}, {prefix}{alt}{params}"
        else:
          if alts == None: alts = f"{prefix}{command.full_parent_name} {alt}{params}"
          else: alts = f"{alts}, {prefix}{command.full_parent_name} {alt}{params}"
      embed = discord.Embed(title=f"`{prefix}{command.qualified_name}{params}`", color=get_client_color(ctx))
      content = ""
      if not alts is None: content += "**Alternate:**\n```"+alts+"```\n"
      if command.description == "":
        if not command.brief is None: content += "**Description:\n**"+command.brief+"\n\n"
      else: content += "**Description:**\n"+command.description+"\n\n"
      if not command.usage is None: content=(content+command.usage).format(prefix)+"\n"
      if not command.help is None: content += ":bulb: **Tip:**\n"+command.help+"\n\n"
      if command.cog_name is None: cog = "📁 Other"
      else: cog = categories[command.cog_name]
      if not command.full_parent_name == "": content += f"**Subcommand of:**\n```{prefix}"+command.full_parent_name+"```\n"
      try:
        subcommands = None
        for subcmd in command.commands:
          params = ""
          for item in subcmd.clean_params:
            params = f"{params} <{item}>"
          if subcommands == None: subcommands = prefix+subcmd.qualified_name+params
          else: subcommands = subcommands+"\n"+prefix+subcmd.qualified_name+params
        content += "**Subcommands:**\n```"+subcommands+"```\n"
      except Exception: pass
...
async def setup(ctx):
  ...
  embed = discord.Embed(title="Step 1", description=f"**{ctx.author.name}, please answer the following questions!** You have 5 minutes for each question.", color=get_client_color(ctx))
...
async def prefix(ctx):
  if ctx.invoked_subcommand is None:
    ...
    embed = discord.Embed(title=f"{len(prefixes)} Prefixes", description=description, color=get_client_color(ctx))
...
async def add(ctx, *,  prefix):
  if prefix.startswith("/"):
    await ctx.send("TimMcBot prefixes can't start with / - that prefix is reserved to slash commands!")
  elif len(prefix) > 15:
...
async def remove(ctx, *,  prefix):
  if prefix == "<@!800377812699447306> ":
    ...
    if prefix in prefixes:
      if len (prefixes) == 1:
        await ctx.send(f"There must be at least one prefix beside {client.user.mention}!")
      else:
...
#slash-commands:
@slash.slash(guild_ids=[806272849458495489],
...
async def potionz(ctx):
    await ctx.send(embed=discord.Embed(title="Potionz", description="Potionz is lacking awesomeness.\nIt brings users helplessness.\nMy goodness, it brought me sadness!\nHow're you going to enhance this evil business?\n\n*This amazing poem was written by the Scratcher @icmy123 and modified by @TimMcCool*", color=discord.Color.green()))
...
async def get_invites(ctx, user):
  embed = discord.Embed(title="Invites", description = "This member hasn't created any invites yet!", color=get_client_color(ctx))
  guild_invites = await ctx.guild.invites()
  invited_users = 0
  for invite in guild_invites:
    if invite.inviter.id == user.id:
      embed.add_field(name="discord.gg/"+invite.id, value=f"Created at: {invite.created_at.strftime('%m/%d/%Y')}\nChannel: {invite.channel.mention}\nUses: {invite.uses}\n")
...
async def dm(ctx, receiver: discord.User, *, message):
  ...
  if success is True:
    await ctx.send(ctx.author.mention, embed=discord.Embed(title="DM sent! 📬", description=f"Your DM was sucessfully sent to **{receiver}**", color=discord.Color.green()))
  else:
    embed=discord.Embed(title="This user doesn't accept DMs from me. :frowning:", description="Therefore, I wasn't able to send your message.", color=discord.Color.red())
...
async def _dm(ctx, receiver : discord.User, message):
  ...
  if success is True:
    ...
    await ctx.send(f"Failed to send DM because **{receiver}** doesn't accept DMs from me", hidden=True)
...
async def send_dm(ctx, receiver : discord.User, message):
  try:
    ...
    embed = discord.Embed(title="Direct Message", description=message, color=discord.Color.random(), timestamp=datetime.datetime.now())
...
@client.command(brief='Add me to your server!', description="Run the command to add me to your server!")
async def invite(ctx):
  invite = discord.Embed(
      description=
        "[Invite link for server owners](https://discord.com/api/oauth2/authorize?client_id=800377812699447306&permissions=8&scope=bot)\n[Invite link for server admins](https://discord.com/api/oauth2/authorize?client_id=800377812699447306&permissions=4294967287&scope=bot%20applications.commands)",
      color=get_client_color(ctx))
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.CommandNotFound): return
      ...
      if isinstance(error.original, discord.Forbidden):
        ...
        await ctx.send(embed=discord.Embed(title="Error", description="Something went wrong!", color = discord.Colour.red()))
  ...
  if not str(message.author.id) in bans and not message.content == "":
    if not message.author.bot:
      if str(message.channel.type) == "private":
        ...
        nachricht = discord.Embed(title="Direct Message to TimMcBot",
                                  description=message.content,
                                  color=discord.Colour.gold())
        nachricht.set_author(name=str(
            message.author), icon_url=message.author.avatar_url)
